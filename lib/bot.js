...
class Bot {
  constructor(options) {
    ...
    this.muteBots = options.muteBots || [];
  }
  ...
  sendToIRC(message) {
    ...
    if (ircChannel) {
      ...
      if (this.isCommandMessage(text)) {
        ...
        if (this.muteBots.indexOf(message.bot_id) !== -1) return;
        if (message.attachments && message.attachments[0]) {
          var atitle = message.attachments[0].title,
              atext = message.attachments[0].text,
              color = message.attachments[0].color || "none";
          if (atitle && atext) {
            text = `${atitle}: ${atext} (#${color})`;
          } else {
            text = `Bot: ${message.attachments[0].fallback}`;
          }
        } else {
          text = `Bot: ${message.username}: ${text}`;
        }
      }
